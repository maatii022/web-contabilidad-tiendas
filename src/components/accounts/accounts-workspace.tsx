...
import { DailyClosingSheet } from '@/components/forms/daily-closing-sheet';
...
export function AccountsWorkspace({
  ...
  const selectedAccount = useMemo(() => {
    return data.accounts.find((account) => account.id === filters.accountId) ?? data.accounts[0] ?? null;
  }, [data.accounts, filters.accountId]);
    ...
    <div className="grid gap-5">
      <section className="surface-card rounded-[32px] p-5 md:p-6 shadow-[0_22px_42px_rgba(60,70,49,0.08)]">
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--foreground-muted)]">Caja y banco</p>
            <h1 className="mt-1 text-[1.6rem] font-semibold tracking-[-0.05em] text-[var(--foreground)]">Tesorería operativa</h1>
            <p className="mt-2 text-sm text-[var(--foreground-soft)]">Vista tipo banca, con saldo, cuentas y movimientos en el centro.</p>
          </div>

          {canManage ? (
            <div className="flex flex-wrap gap-2">
              <button type="button" className={buttonClassName('secondary')} onClick={() => setCreateAccountOpen(true)}>
                <Landmark className="mr-2 h-4 w-4" />Nueva cuenta
              </button>
              <button type="button" className={buttonClassName('secondary')} onClick={() => setClosingOpen(true)}>
                <CreditCard className="mr-2 h-4 w-4" />Cerrar día
              </button>
              <button type="button" className={buttonClassName('primary')} onClick={() => setEntryOpen(true)}>
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_340px]">
          <div className="rounded-[30px] border border-[rgba(123,136,95,0.16)] bg-[linear-gradient(180deg,rgba(110,127,86,0.14),rgba(255,255,255,0.86))] p-5 shadow-[0_20px_34px_rgba(60,70,49,0.09)]">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-medium text-[var(--foreground-soft)]">Disponible</p>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <span className="rounded-full bg-[rgba(85,117,82,0.12)] px-3 py-1.5 text-xs font-semibold text-[var(--success)]">
                    Entradas {formatCurrency(data.totals.inflow, currency)}
                  </span>
                  <span className="rounded-full bg-[rgba(155,91,83,0.12)] px-3 py-1.5 text-xs font-semibold text-[var(--danger)]">
                    Salidas {formatCurrency(data.totals.outflow, currency)}
                  </span>
                </div>
              </div>
              <div className="rounded-[22px] bg-white/82 p-3 text-[var(--accent-strong)] shadow-[0_12px_24px_rgba(60,70,49,0.06)]">
                <Wallet className="h-6 w-6" />
              </div>
            </div>
          </div>

          <div className="rounded-[30px] border border-[rgba(123,136,95,0.16)] bg-white/84 p-5 shadow-[0_16px_28px_rgba(60,70,49,0.06)]">
            <p className="text-sm font-medium text-[var(--foreground-soft)]">Cuenta enfocada</p>
            {selectedAccount ? (
              <>
                <div className="mt-3 flex items-start justify-between gap-3">
                  <div>
                    <p className="text-lg font-semibold text-[var(--foreground)]">{selectedAccount.name}</p>
                    <p className="mt-1 text-xs uppercase tracking-[0.14em] text-[var(--foreground-muted)]">{accountTypeLabel(selectedAccount.type)}</p>
                  </div>
                ...
                <p className="mt-4 text-[2rem] font-semibold tracking-[-0.05em] text-[var(--foreground)]">{formatCurrency(selectedAccount.currentBalance, currency)}</p>
                <p className="mt-2 text-sm text-[var(--foreground-soft)]">
                  {selectedAccount.movementCount} movimientos{selectedAccount.lastEntryDate ? ` · último ${formatDate(selectedAccount.lastEntryDate)}` : ''}
                </p>
              </>
            ) : (
              <p className="mt-3 text-sm text-[var(--foreground-soft)]">No hay cuentas creadas todavía.</p>
            )}
          </div>
        </div>
        ...
        <div className="mt-5 flex gap-3 overflow-x-auto pb-1 no-scrollbar">
          <a
            href="/caja"
            className={cn(
              'bank-chip min-w-[180px] rounded-[24px] px-4 py-4 transition',
              !filters.accountId && 'border-[rgba(110,127,86,0.3)] shadow-[0_16px_30px_rgba(83,99,65,0.12)]'
            )}
          >
          ...
          {data.accounts.map((account) => (
            <a
              key={account.id}
              href={buildAccountHref(account.id, filters)}
              className={cn(
                'bank-chip min-w-[220px] rounded-[24px] px-4 py-4 transition',
                account.id === selectedAccount?.id && 'border-[rgba(110,127,86,0.3)] shadow-[0_16px_30px_rgba(83,99,65,0.12)]'
              )}
            >
        ...
        <form className="mt-5 grid gap-4" method="get">
          ...
          <div className="grid gap-3 lg:grid-cols-2 2xl:grid-cols-[minmax(0,1.55fr)_220px_170px_170px]">
            <label className="grid gap-2 text-sm text-[var(--foreground-soft)]">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Concepto, nota u origen"
                />
              </div>
            </label>
          ...
          <div className="flex flex-col gap-3 border-t border-[rgba(123,136,95,0.12)] pt-4 md:flex-row md:items-center md:justify-between">
            <p className="text-sm text-[var(--foreground-soft)]">Entras viendo el mes actual. El feed se adapta a la cuenta elegida y al rango.</p>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.32fr)_360px]">
        <section className="surface-card rounded-[30px] p-4 md:p-5 shadow-[0_18px_34px_rgba(60,70,49,0.06)]">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--foreground-muted)]">Movimientos</p>
              <p className="mt-1 text-sm text-[var(--foreground-soft)]">Feed principal de tesorería, con entradas, salidas y ajustes.</p>
            </div>
            <span className="rounded-full bg-[rgba(110,127,86,0.1)] px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-[var(--accent-strong)]">
              {data.totals.entryCount} registros
            </span>
          </div>

          {data.entries.length === 0 ? (
            <div className="rounded-[22px] border border-dashed border-[rgba(123,136,95,0.22)] bg-white/55 px-4 py-10 text-center">
              <p className="text-lg font-semibold text-[var(--foreground)]">No hay movimientos con esos filtros.</p>
              <p className="mt-2 text-sm text-[var(--foreground-soft)]">Ajusta el rango o registra el primer movimiento manual.</p>
              {canManage ? (
                <div className="mt-5">
                  <button type="button" className={buttonClassName('primary')} onClick={() => setEntryOpen(true)}>
                ...
                <article key={entry.id} className="dashboard-row rounded-[24px] border border-[rgba(123,136,95,0.12)] bg-white/86 p-4 shadow-[0_14px_24px_rgba(60,70,49,0.05)]">
                  <div className="flex items-start justify-between gap-3">
                    <div className="grid gap-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="rounded-full bg-[rgba(110,127,86,0.08)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--accent-strong)]">
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-3">
                      <p className={cn('text-2xl font-semibold tracking-[-0.04em]', amountTone(entry.signedAmount))}>
                        {entry.signedAmount > 0 ? '+' : ''}{formatCurrency(entry.signedAmount, currency)}
                      </p>
                      {canManage && entry.sourceType === 'manual' ? (
                        <form>
                          ...
                          <Button
                            type="submit"
                            variant="ghost"
                            className="h-10 px-3 text-[var(--danger)] hover:bg-[rgba(155,91,83,0.08)] hover:text-[var(--danger)]"
        ...
        <div className="grid gap-5">
          <section className="surface-card rounded-[30px] p-4 md:p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--foreground-muted)]">Cuentas</p>
                <p className="mt-1 text-sm text-[var(--foreground-soft)]">Renombra, cambia el tipo o saca de uso una cuenta sin romper historial.</p>
              </div>
            </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-[var(--foreground-soft)]">
                          {account.movementCount} movimientos{account.lastEntryDate ? ` · último ${formatDate(account.lastEntryDate)}` : ''}
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        ...
                        {!canDelete ? (
                          <form>
                            ...
                            <input type="hidden" name="returnPath" value={buildBaseHref(filters)} />
                            <Button type="submit" variant="ghost" formAction={deactivateAccountAction}>
                              <ShieldMinus className="mr-2 h-4 w-4" />Desactivar
                            </Button>
                          </form>
                        ) : (
                          <form>
                            <input type="hidden" name="accountId" value={account.id} />
                            <input type="hidden" name="returnPath" value={buildBaseHref(filters)} />
                            <Button type="submit" variant="ghost" className="text-[var(--danger)] hover:bg-[rgba(155,91,83,0.08)] hover:text-[var(--danger)]" formAction={deleteAccountAction}>
          ...
          <section className="surface-card rounded-[30px] p-4 md:p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--foreground-muted)]">Cierres diarios</p>
                <p className="mt-1 text-sm text-[var(--foreground-soft)]">Últimos cierres guardados de cada cuenta.</p>
              </div>
            </div>
            ...
            {data.closings.length === 0 ? (
              <div className="rounded-[22px] border border-dashed border-[rgba(123,136,95,0.22)] bg-white/55 px-4 py-8 text-center">
                ...
                <p className="mt-2 text-sm text-[var(--foreground-soft)]">Úsalos para fijar apertura y cierre del día.</p>
              </div>
            ) : (
              <div className="grid gap-3">
                {data.closings.slice(0, 6).map((closing) => (
                  <article key={closing.id} className="rounded-[22px] border border-[rgba(123,136,95,0.12)] bg-white/82 p-4 shadow-[0_12px_22px_rgba(60,70,49,0.04)]">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="rounded-full bg-[rgba(110,127,86,0.08)] p-2 text-[var(--accent-strong)]">
                        <CreditCard className="h-4 w-4" />
                      </div>
                    </div>
                    <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <p className="text-[var(--foreground-soft)]">Apertura</p>
                        <p className="mt-1 font-semibold text-[var(--foreground)]">{formatCurrency(closing.openingBalance, currency)}</p>
                      </div>
                      <div>
                        <p className="text-[var(--foreground-soft)]">Cierre</p>
                        <p className="mt-1 font-semibold text-[var(--foreground)]">{formatCurrency(closing.closingBalance, currency)}</p>
                      </div>
                      <div className="inline-flex items-center gap-2 text-[var(--success)]">
                        <ArrowUpRight className="h-4 w-4" />{formatCurrency(closing.inflowTotal, currency)}
                      </div>
                      <div className="inline-flex items-center gap-2 text-[var(--danger)]">
                        <ArrowDownLeft className="h-4 w-4" />{formatCurrency(closing.outflowTotal, currency)}
                      </div>
                    </div>
                  </article>
                ))}
      ...
      <AccountEntrySheet
        ...
        presetAccountId={selectedAccount?.id ?? filters.accountId}
        returnPath={buildBaseHref(filters)}
      />
      <DailyClosingSheet
        open={closingOpen}
