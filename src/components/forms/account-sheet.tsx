...
import { accountFormInitialState, type AccountSummaryItem } from '@/features/accounts/types';
...
export function AccountSheet({
  ...
  const sessionKey = useSheetSessionKey(open, account?.id ?? 'new');
  ...
  return (
    ...
    >
      {open ? <AccountSheetForm key={`${account?.id ?? 'new'}-${sessionKey}`} onClose={onClose} account={account} returnPath={returnPath} /> : null}
    </Sheet>
  );
}
