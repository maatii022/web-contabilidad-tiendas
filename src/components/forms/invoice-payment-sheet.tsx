...
export function InvoicePaymentSheet({ open, onClose, catalogs, invoice, payment, installment, returnPath, mode = 'create' }: { open: boolean; onClose: () => void; catalogs: InvoiceCatalogs; invoice: InvoiceListItem | null; payment?: InvoicePaymentItem | null; installment?: InvoiceInstallmentItem | null; returnPath: string; mode?: InvoicePaymentSheetMode; }) {
  const sessionKey = useSheetSessionKey(open, `${invoice?.id ?? 'none'}-${payment?.id ?? 'new'}-${installment?.id ?? 'all'}`);
  const title = mode === 'edit' ? 'Editar pago' : 'Registrar pago';
  const description = invoice ? (mode === 'edit' ? `Edita el pago de la factura ${invoice.invoiceNumber}.` : `Factura ${invoice.invoiceNumber} · pendiente ${invoice.pendingAmount.toFixed(2)} €`) : 'Registra un pago en una cuenta existente.';

  return (
    <Sheet open={open} onClose={onClose} title={title} description={description}>
      {open && invoice ? <InvoicePaymentSheetForm key={`${invoice.id}-${payment?.id ?? 'new'}-${installment?.id ?? 'all'}-${sessionKey}`} onClose={onClose} catalogs={catalogs} invoice={invoice} payment={payment} installment={installment} returnPath={returnPath} mode={mode} /> : null}
    </Sheet>
  );
}

function InvoicePaymentSheetForm({ onClose, catalogs, invoice, payment, installment, returnPath, mode }: { onClose: () => void; catalogs: InvoiceCatalogs; invoice: InvoiceListItem; payment?: InvoicePaymentItem | null; installment?: InvoiceInstallmentItem | null; returnPath: string; mode: InvoicePaymentSheetMode; }) {
  ...
  const defaultAmount = activePayment ? activePayment.amount.toFixed(2) : installment ? installment.pendingAmount.toFixed(2) : invoice.pendingAmount.toFixed(2);
  ...
  return (
    <form action={formAction} className="grid gap-4">
      ...
      <div className="mt-2 flex items-center justify-end gap-3 border-t border-[rgba(123,136,95,0.12)] pt-4">
        ...
        <Button type="submit" disabled={pending}>{pending ? 'Guardando...' : mode === 'edit' ? 'Guardar cambios' : 'Registrar pago'}</Button>
      </div>
    </form>
  );
}
