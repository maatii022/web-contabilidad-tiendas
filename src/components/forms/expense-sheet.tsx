...
export function ExpenseSheet({
  ...
  return (
    <Sheet
      open={open}
      onClose={onClose}
      title={title ?? (expense ? 'Editar gasto' : 'Nuevo gasto')}
      description={expense ? 'Actualiza importe, cuenta o adjunto sin salir del panel.' : 'Registra un gasto y, si procede, deja el movimiento reflejado en la cuenta elegida.'}
    >
      {open ? (
        <form key={expense?.id ?? 'new-expense'} action={formAction} className="grid gap-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Fecha" error={state.fieldErrors?.expenseDate}>
              <div className="relative">
                ...
                <input
                  className={inputClassName(Boolean(state.fieldErrors?.expenseDate)) + ' pl-11'}
                  type="date"
                  name="expenseDate"
                  defaultValue={expense?.expenseDate ?? todayIso()}
                />
              </div>
            </Field>
              ...
              <div className="grid gap-2">
                ...
                {!hasCategories ? (
                  <Link href="/configuracion" className="text-xs font-medium text-[var(--accent-strong)] underline-offset-4 hover:underline">
                    Crear categorías en Configuración
                  </Link>
                ) : null}
              </div>
            </Field>
            ...
            <div className="grid gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  name="vendorName"
                  placeholder={catalogs.vendors.length ? 'Escribe el nombre del proveedor' : 'No hay proveedores todavía'}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Base" error={state.fieldErrors?.baseAmount}>
              <input className={inputClassName(Boolean(state.fieldErrors?.baseAmount))} type="number" step="0.01" min="0" name="baseAmount" defaultValue={expense?.baseAmount ?? 0} />
            </Field>
            <Field label="Impuestos" error={state.fieldErrors?.taxAmount}>
              <input className={inputClassName(Boolean(state.fieldErrors?.taxAmount))} type="number" step="0.01" min="0" name="taxAmount" defaultValue={expense?.taxAmount ?? 0} />
            </Field>
          </div>

          <Field label="Referencia" error={state.fieldErrors?.reference}>
            <input className={inputClassName(Boolean(state.fieldErrors?.reference))} name="reference" placeholder="Ej. ticket 1938" defaultValue={expense?.reference ?? ''} />
          </Field>

          <Field label="Notas" error={state.fieldErrors?.notes}>
            <textarea className={textareaClassName(Boolean(state.fieldErrors?.notes))} name="notes" placeholder="Detalle breve para identificar el gasto" defaultValue={expense?.notes ?? ''} />
          </Field>
          ...
          <Field label="Adjunto" error={state.fieldErrors?.attachment} hint="PDF o imagen, hasta 10 MB.">
            <div className="rounded-[24px] border border-dashed border-[rgba(123,136,95,0.22)] bg-white/60 p-4">
              <div className="flex items-center gap-3 text-sm text-[var(--foreground-soft)]">
                ...
                <span>{expense ? 'Añadir adjunto adicional' : 'Adjuntar ticket o factura'}</span>
              </div>
