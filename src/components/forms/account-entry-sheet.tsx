...
export function AccountEntrySheet({
  ...
  const sessionKey = useSheetSessionKey(open, presetAccountId ?? 'entry');

  return (
    <Sheet open={open} onClose={onClose} title="Ajuste manual" description="Solo para apuntes puntuales fuera del flujo rápido de caja.">
      {open ? <AccountEntrySheetForm key={`${presetAccountId ?? 'default'}-${sessionKey}`} onClose={onClose} catalogs={catalogs} returnPath={returnPath} presetAccountId={presetAccountId} /> : null}
    </Sheet>
  );
}

function AccountEntrySheetForm({ onClose, catalogs, returnPath, presetAccountId }: { onClose: () => void; catalogs: CashCatalogs; returnPath: string; presetAccountId?: string }) {
    ...
    <form action={formAction} className="grid gap-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="Tipo" error={state.fieldErrors?.type} hint="En ajuste puedes usar importe positivo o negativo.">
          <select className={inputClassName(Boolean(state.fieldErrors?.type))} name="type" defaultValue="adjustment">
      ...
      <Field label="Concepto" error={state.fieldErrors?.concept}>
        <input className={inputClassName(Boolean(state.fieldErrors?.concept))} name="concept" placeholder="Ej. ajuste por descuadre" />
      </Field>
      ...
      <div className="mt-2 flex items-center justify-end gap-3 border-t border-[rgba(123,136,95,0.12)] pt-4">
        ...
        <Button type="submit" disabled={pending}>{pending ? 'Guardando...' : 'Guardar ajuste'}</Button>
      </div>
    </form>
  );
}
